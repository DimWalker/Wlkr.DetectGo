...
def load_coco_data(raw_dir, json_file_name):
    with open(os.path.join(raw_dir, json_file_name), "r", encoding="utf-8") as f:
        coco_data = json.load(f)
    return coco_data


def load_label(raw_dir, label_file_name):
    with open(os.path.join(raw_dir, label_file_name), "r", encoding="utf-8") as f:
        lines = f.readlines()
    sc_wp_list = []
    for line in lines:
...
def warp_back(model, raw_dir, dataset_type, json_file_name, label_file_name):
    raw_dir = os.path.join(raw_dir, dataset_type)
    output_dir = "../output/diagram_det_rec_dataset/" + dataset_type
    abs_dir = os.path.dirname(os.path.dirname(__file__))
    abs_dir = abs_dir.replace("/", "\\")
    abs_dir = os.path.join(abs_dir, "output", "diagram_det_rec_dataset", dataset_type)
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    coco_data = load_coco_data(raw_dir, json_file_name)
    sc_wp_list = load_label(raw_dir, label_file_name)
    ...
    for img_path in input_list:
        ...
        M, save_name = board_warp_back(model, img_path, output_dir)
        if M is None:
            continue
        line = find_row_segmentation(coco_data, sc_wp_list, bn, row_cate_id, M)
        if line is not None:
            res_list.append(f"{dataset_type}/{save_name}\t{line}\n")
...
def warp_back_straight(cnt_limit, ship_cnt, raw_dir, dataset_type):
    output_dir = "../output/diagram_det_rec_dataset/" + dataset_type
    abs_dir = os.path.dirname(os.path.dirname(__file__))
    abs_dir = abs_dir.replace("/", "\\")
    abs_dir = os.path.join(abs_dir, "output", "diagram_det_rec_dataset", dataset_type)
    ...
    for img_path in input_list:
        ...
        if line is not None:
            res_list.append(f"{dataset_type}/{bn}\t{line}\n")
...
def board_warp_back(model, image_path, output_dir, skip_save=None):
